...
- カラー(彩度) — 色相は変えずに彩度だけを調整します（0%で白、100%で現在の色の原色）
- カスタムコマンド

### インジケーター表示

**カラー(RGB)**・**カラー(彩度)**・**色温度**を選択している場合、インジケーターバーの塗りつぶし部分は実際の光の色を、空の部分（トラック）は同じ色に黒を60%重ねた（明度40%まで落とした）色を表示します。明るさ・カスタムコマンドの場合は、実際に発光する色が特定できないため、既定の配色のままになります。

### 実機との同期

テープライトコントロールは、既定20秒間隔（設定画面で最短5秒まで短縮可能）で実機の状態を定期的に取得するほか、シーン実行直後にも取得し直します。これにより、SwitchBotアプリ・物理リモコン・シーンなど、プラグイン以外からの変更（カラー(RGB)を含む）もダイヤル表示に反映されます。ダイヤルは色相環上の102色（すべて彩度100%・明度100%の原色）しか表示できないため、それ以外の色（例：暗めの青、彩度の低い薄い青など）は、色相が最も近い原色に丸めて表示します（例：暗い青も薄い青も、青の原色として表示されます）。「カラー(彩度)」ダイヤルも同様に、実機の色から基準となる色相を同期し、その色相と彩度から求めた混色をインジケーターに反映します。
